const APP_CACHE_KEY = "dividendos-pwa:last-etf-result";
...
async function bootstrap() {
  ...
  try {
    ...
    buildTickerPills(["BEST11"]);
    setFeedback(
      "Nao foi possivel carregar a lista de ETFs locais. O app ainda pode funcionar para arquivos ja gerados.",
      true
    );
  }
...
async function fetchStockData(ticker) {
  ...
  const dataPath = manifestEntry?.path || `./data/etfs/${ticker}.json`;
  const response = await fetch(dataPath, { cache: "no-store" });

  if (response.status === 404) {
    throw new Error(
      `O ETF ${ticker} nao possui arquivo local gerado. Atualize a base local para incluí-lo.`
    );
  }
...
function buildViewModel({ stockData, ticker, years, percent }) {
  ...
  if (!filteredDividends.length) {
    throw new Error(`Nao existem rendimentos validos para ${ticker} nos ultimos ${years} anos.`);
  }
  ...
  return {
    ticker,
    shortName: stockData.fundName || stockData.companyName || ticker,
    longName: stockData.fundName || stockData.companyName || ticker,
    years,
    percent,
    currentPrice,
    totalDividends,
  ...
  const cards = [
    {
      ...
      body: `${model.years} anos com ${model.yearlyTotals.length} ano(s) com rendimentos validos.`,
    },
    {
